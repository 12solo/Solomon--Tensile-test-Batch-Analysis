...
st.title("Solomon Tensile Suite v3.2")
...
if uploaded_files:
    all_results = []
    fig = go.Figure()
    common_strain = np.linspace(0, 1500, 1000) 
    all_stresses = []
    ...
    for file in uploaded_files:
        ...
        if np.sum(mask_e) < 3:
            st.warning(f"⚠️ {file.name}: Check units. Max Strain recorded: {raw_strain.max():.1f}%")
            continue
        ...
        if apply_zeroing:
            ...
            mask_pos = strain >= 0
            strain, stress = strain[mask_pos], raw_stress[mask_pos]
            force_vals = df[f_col].values[mask_pos]
            disp_vals = disp_mm[mask
